...
class LLMManager:
    ...
    def _load_settings(self) -> Dict[str, Any]:
        ...
        if self.settings_file.exists():
            try:
                ...
                logger.warning(f"Не удалось загрузить файл настроек: {e}")
    ...
    def _initialize_provider(self):
        try:
            ...
            if api_key:
                ...
                logger.info(f"Инициализирован провайдер {provider_type.value}, модель: {model_name}")
            else:
                logger.warning(f"Не найден API-ключ для {provider_type.value}")
                
        except Exception as e:
            logger.error(f"Не удалось инициализировать провайдер: {e}")
    ...
    def set_provider(self, provider_type: ProviderType, api_key: str, model_name: str):
        try:
            ...
            logger.info(f"Переключено на провайдер {provider_type.value}, модель: {model_name}")
        except Exception as e:
    ...
    def call(self, prompt: str, input_data: Any = None, **kwargs) -> str:
        if not self.current_provider:
            raise ValueError("Не настроен провайдер LLM. Пожалуйста, укажите API-ключ на странице настроек")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(f"Вызов LLM окончательно не удался после {max_retries} попыток.")
                    raise
                logger.warning(f"Попытка {attempt + 1} не удалась, готовим повтор: {str(e)}")
                import time
    ...
    def parse_json_response(self, response: str) -> Any:
        if not self.current_provider:
            raise ValueError("Не настроен провайдер LLM")
